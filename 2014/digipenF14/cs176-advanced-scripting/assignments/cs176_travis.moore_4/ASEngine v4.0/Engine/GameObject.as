...
package Engine
{
	...
	public class GameObject
	{
		...
		*/
		/*******************************************************************************/
		public function GameObject(displayobject_:DisplayObject, 
								   nPosX_:Number = 0,
								   nPosY_:Number = 0, 
								   iType_:int = 0, 
		...
		*/
		...
		{
			//bPhysicsOn == true to go through code
			//create that physicsinfo if it was not created before (could be created on or off during the game(W or Q).
			//call this upon object creation? set this on object creation bPhysicsOn.
			this.bPhysicsOn = true;
			
			if(this.physicsinfo == null)
			{
				this.physicsinfo = new PhysicsInfo(new Point());
			}
		}
		...
		*/
		...
		{
			//will not clear physicsinfo, only say that the bPhysicsOn is false
		...
		*/
		/*************************************************************************/
		final public function SetPhysicsProperties(nMass_:Number, 
												   pVelocityDirection_:Point,
											 	   nVelocityMagnitude_:Number):void
		{
			//use this to define the physics properties.
			//create a physicsinfo if it does not yet exist (does not change bPhysicsOn to true).
			if(this.physicsinfo == null)
			{
				this.physicsinfo = new PhysicsInfo(new Point());
			}
			
			this.physicsinfo.nMass = nMass_;
			this.physicsinfo.pVelocityDirection = pVelocityDirection_;
			this.physicsinfo.nVelocityMagnitude = nVelocityMagnitude_;
		}
