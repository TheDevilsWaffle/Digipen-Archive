...
Shader "CloverSwatch/CustomBarrier"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			{
				//return saturate(abs(frac(offset + t) * 1 - 1) + yOffset);
				return saturate(abs(frac(t) * 1 - 1) + yOffset);
			}

			fixed4 texColor(v2f i, float rim)
			{
				fixed4 mainTex = tex2D(_MainTex, i.uv);
				mainTex.r *= triWave(_Time.x * 10, abs(i.objectPos.x) * -1, -0.5) * 4;
				// I ended up saturaing the rim calculation because negative values caused weird artifacts
				mainTex.g *= saturate(rim) * (sin(_Time.z + mainTex.b * 1) + 1);
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float northPole = (i.objectPos.y - 0.45) * 0.5;
				float glow = max(max(intersect, rim), northPole);

				fixed4 glowColor = fixed4(lerp(_Color.rgb, fixed3(1, 1, 1), pow(glow, 1)), 1);
