	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 texColor(v2f i, float rim)
			{
				...
				mainTex.r *= triWave(_Time.x * 5, abs(i.objectPos.x) * -1, -0.5) * 2;
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float rim = 1 - abs(dot(i.normal, normalize(i.viewDir))) * 10;
				float northPole = (i.objectPos.y) * 0.5;
				float glow = max(max(intersect, rim), northPole);

				fixed4 glowColor = fixed4(lerp(_Color.rgb, fixed3(0.1,0.1,0.1), pow(glow, 0.5)), 0.5);
