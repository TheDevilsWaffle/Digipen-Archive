...
class py_character_movement():
    ...
    def OnLogicUpdate(self, updateEvent):
        ...
        #If the player leaves the left side of the screen, they emerge from the right
        if(currentTranslation.x <= -18):
            self.Owner.Transform.Translation += Vec3 (36,0,0)
            print("woosh")
            
        #If the player leaves the right side of the screen, they emerge from the left
        if(currentTranslation.x >= 18):
            self.Owner.Transform.Translation += Vec3 (-36,0,0)
            print("weeee")
    ...
    def CanJump(self):
        ...
        #TO DO - ADD LOGIC FOR IF ON LADDERS/ROPES/ETC...
